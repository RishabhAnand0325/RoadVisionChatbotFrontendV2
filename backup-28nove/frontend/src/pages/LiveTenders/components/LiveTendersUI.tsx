...
export default function LiveTendersUI({
  ...
  const [viewMode, setViewMode] = useState<'all' | 'wishlisted'>('all');
    ...
    report.queries.forEach((query) => {
      ...
      query.tenders.forEach((tender) => {
        if ((tender.tender_name.toLowerCase().includes(searchQuery.toLowerCase())
          ...
          // Apply wishlist filter if needed
          if (viewMode === 'wishlisted' && !isInWishlist(tender.id)) {
            return;
          }
  ...
  useEffect(() => {
    filterReport()
  }, [searchQuery, report, minPrice, viewMode])
        ...
        <Card className="p-4 border-2">
          <div className="flex flex-col gap-4">
            ...
            <div className="flex gap-2">
              ...
              <Button
                variant={viewMode === 'wishlisted' ? 'default' : 'outline'}
                onClick={() => setViewMode('wishlisted')}
                className="flex-1 sm:flex-none gap-2"
              >
                <Star className={`h-4 w-4 ${viewMode === 'wishlisted' ? 'fill-current' : ''}`} />
                Wishlisted
              </Button>
            </div>
