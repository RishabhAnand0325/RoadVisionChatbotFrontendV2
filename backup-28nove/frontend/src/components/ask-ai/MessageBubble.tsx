...
export function MessageBubble({ message }: MessageBubbleProps) {
  ...
  return (
    <div
      className={`flex gap-3 animate-in fade-in slide-in-from-bottom-5 duration-300 w-full ${message.sender === "user" ? "flex-row-reverse justify-end" : "justify-start"
        }`}
    >
      ...
      <div className="flex-1 flex flex-col">
          ...
          <div className="max-w-2xl lg:max-w-4xl w-full text-sm text-current">
            <ReactMarkdown
              ...
              components={{
                ...
                pre: ({ node, ...props }) => (
                  <pre
                    className="w-full overflow-x-auto bg-gray-100 dark:bg-gray-800 p-2 rounded"
                    {...props}
                  />
                ),
                code: ({ node, ...props }) => (
                  <code
                    className="inline-block rounded bg-gray-100 dark:bg-gray-800 px-1 py-0.5 font-mono text-sm"
                    {...props}
                  />
                ),
                img: ({ node, ...props }) => (
        ...
        <div
          className={`text-xs text-muted-foreground mt-1.5 ${message.sender === "user" ? "text-right" : ""
            }`}
        >
