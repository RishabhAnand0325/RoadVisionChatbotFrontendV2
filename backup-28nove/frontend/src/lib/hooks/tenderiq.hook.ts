...
export const useReportStream = (run_id?: string, dateRange?: string) => {
  ...
  useEffect(() => {
    // Generate cache key
    const cacheKey = `tenderiq_report_${run_id || 'latest'}_${dateRange || 'default'}`;
    
    // Check cache first
    const cachedData = sessionStorage.getItem(cacheKey);
    if (cachedData) {
      try {
        const parsedReport = JSON.parse(cachedData) as Report;
        console.log("Loaded report from cache:", cacheKey);
    ...
    const handleComplete = (event: MessageEvent) => {
      ...
      setReport((currentReport) => {
        if (currentReport) {
           try {
             sessionStorage.setItem(cacheKey, JSON.stringify(currentReport));
             console.log("Saved report to cache:", cacheKey);
           } catch (e) {
             console.error("Failed to save report to cache (likely quota exceeded)", e);
           }
        }
